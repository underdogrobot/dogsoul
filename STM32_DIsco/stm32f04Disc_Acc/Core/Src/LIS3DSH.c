...
static float __Full_Scale = 2.0f;
...
LIS3DSH_DataRaw LIS3DSH_GetDataRaw1(void)
{
	...
	tempDataRaw.x = ((spiBuf[1] << 8) + spiBuf[0]);
	tempDataRaw.y = ((spiBuf[2] << 8) + spiBuf[3]);
	tempDataRaw.z = ((spiBuf[4] << 8) + spiBuf[5]);
...
LIS3DSH_DataScaled LIS3DSH_GetDataScaled(void)
{
	...
	tempScaledData.x = (tempRawData.x * __Full_Scale * __X_Scale / 32768) + 0.0f - __X_Bias;
	tempScaledData.y = (tempRawData.y * __Full_Scale * __Y_Scale / 32768) + 0.0f - __Y_Bias;
	tempScaledData.z = (tempRawData.z * __Full_Scale * __Z_Scale / 32768) + 0.0f - __Z_Bias;
